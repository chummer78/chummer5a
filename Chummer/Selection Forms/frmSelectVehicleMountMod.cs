...
namespace Chummer
{
	public partial class frmSelectVehicleMountMod : Form
	{
		...
		public frmSelectVehicleMountMod(Character objCharacter, bool blnCareer = false)
		{
			InitializeComponent();
		...
		private void frmSelectVehicleMountMod_Load(object sender, EventArgs e)
		{
			BuildModList();
			...
			if (_strInputFile == "weapons")
				this.Text = LanguageManager.Instance.GetString("Title_SelectVehicleMountMod_Weapon");
		}
		...
		private void BuildModList()
		{
			...
			foreach (XmlNode objXmlMod in objXmlModList)
			{
				...
				*/
				...
				{
					foreach (XmlNode node in objXmlMod["tags"].ChildNodes)
					{
						if (node.InnerText == "mountmod")
							blnAdd = true;
					}
				}
				if (blnAdd)
